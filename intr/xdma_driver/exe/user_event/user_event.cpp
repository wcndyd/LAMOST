...
int __cdecl main(int, char* argv[]) {
    ...
    try {
		...
		const auto user_dev_path = dev_paths[0] + "\\user";
		device_file dev_user(user_dev_path, GENERIC_WRITE);
		DWORD user_irq_en[1];
		user_irq_en [0]= 0xffff0000;
		dev_user.write(0x00004, 4, (BYTE*)user_irq_en);

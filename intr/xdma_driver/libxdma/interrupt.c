...
// Deferred interrupt service handler
{
	...
	PUCHAR intr_writeAddr = (PUCHAR)irq->xdma->bar[irq->xdma->userBarIdx];
	intr_writeAddr = intr_writeAddr + 8;
	WRITE_REGISTER_BUFFER_ULONG((volatile ULONG*)intr_writeAddr, (PULONG)&irq->userIrqPending, 1);
...
// Deferred interrupt service handler
{
	...
	PUCHAR intr_writeAddr = (PUCHAR)irq->xdma->bar[irq->xdma->userBarIdx];
	intr_writeAddr = intr_writeAddr + 8;
	volatile ULONG irq_request = BIT_N(irq->eventId);
	WRITE_REGISTER_BUFFER_ULONG((volatile ULONG*)intr_writeAddr, (PULONG)&irq_request, 1);

...
int read_image_file_bmp (const char* filename, ImageData *imagedata)
{
    ...
    while(1) /* not a loop */
    {
        ...
        i = bmpHeader.height * bmpHeader.width * 3;
        k = bmpHeader.height * bmpHeader.width * 4;
        pixelArray = (unsigned char*)malloc(k);
        colour = pixelArray + k;
        ...
        if(bmpHeader.bitsPerPixel == 24)
        {
            for(j = 0; j < bmpHeader.height; j++)
            {
                ...
                {
                    colour[i*4+0] = content[i*3+0];
                    colour[i*4+1] = content[i*3+1];
                    colour[i*4+2] = content[i*3+2];
                    colour[i*4+3] = 0;
                }
